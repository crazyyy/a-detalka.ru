...
jQuery.fn.daterangepicker = function(settings){
	...
	if (typeof window.langDateRange == "undefined") {
		// Текстовые фразы вынесены отдельно безопасным способом.
		// Для работы локализации, необходимо вместо подключения js файла вывести шаблон js.daterangepicker.tpl
		var langDateRange = {
			today: 'Сегодня',
			lastSevenDay: 'За последние 7 дней',
			thisMonth: 'За этот месяц',
			thisYear: 'За этот год',
			thisLastYear: 'За последний год',
		...
		var langDateRange = window.langDateRange;
	}

	//defaults
	var options = jQuery.extend({
		presetRanges: [
			{text: langDateRange.today, dateStart: 'today', dateEnd: 'today' },
			{text: langDateRange.lastSevenDay, dateStart: 'today-7days', dateEnd: 'today' },
			{text: langDateRange.thisMonth, dateStart: function(){ return Date.parse('today').moveToFirstDayOfMonth();  }, dateEnd: 'today' },
			{text: langDateRange.thisYear, dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
			//extras:
			{text: langDateRange.previousMonth, dateStart: function(){ return Date.parse('1 month ago').moveToFirstDayOfMonth();  }, dateEnd: function(){ return Date.parse('1 month ago').moveToLastDayOfMonth();  } },
			{text: langDateRange.thisLastYear, dateStart: function(){ return Date.parse('1 year ago');  }, dateEnd: 'today' }
		...
		presets: {
			//specificDate: langDateRange.specificDate,
			//allDatesBefore: langDateRange.allDatesBefore,
			//allDatesAfter: langDateRange.allDatesAfter,
			dateRange: langDateRange.dateRange
		},
		rangeStartTitle: langDateRange.rangeStartTitle,
		rangeEndTitle: langDateRange.rangeEndTitle,
		nextLinkText: langDateRange.nextLinkText,
		prevLinkText: langDateRange.prevLinkText,
		doneButtonText: 'ОК',
